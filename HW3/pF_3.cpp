...
void householder_qr(const vector<vector<double>> &A, vector<vector<double>> &Q, vector<vector<double>> &R) {
    ...
    Q = vector<vector<double>>(n, vector<double>(n, 0.0)); // unit matrix
    for (int i = 0; i < n; ++i) {
        Q[i][i] = 1.0;
    }
    ...
    for (int k = 0; k < m; ++k) {
        ...
        for (int i = 0; i < n; ++i) {
            for (int j = 0; j < n; ++j) {
                H_k_full[i][j] = (i >= k && j >= k ? H_k[i - k][j - k] : (i == j ? 1.0 : 0.0));
            }
        }
...
int main() {
    ...
    for(degree=5;degree<=8;degree++) {
        ...
        householder_qr(mt, mt_Q, mt_R);

        vector<vector<double>> Q_T = transpose_matrix(mt_Q);
        vector<vector<double>> right = multiply_matrix(Q_T, y);
        ...
        result.push_back(c);
        rel_error.push_back(relative_error(c, ori_c));
